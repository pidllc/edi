...
BEGIN
    ...
    -- Use STUFF to remove trailing comma+space properly
    IF LEN(@result) > 1 SET @result = STUFF(@result, LEN(@result) - 1, 2, '');
    RETURN @result;
END
GO
...
BEGIN
    ...
    CLOSE col_cursor;
    DEALLOCATE col_cursor;
    IF LEN(@result) > 0 SET @result = STUFF(@result, LEN(@result), 1, '');
    RETURN @result;
END
GO
