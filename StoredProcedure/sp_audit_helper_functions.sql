...
-- ============================================================
-- Helper functions for sp_audit_reconstruct_sql
-- ============================================================

-- fn_json_to_values: Converts JSON to VALUES clause
-- e.g. '{"col1":"val1","col2":123}' → ''val1',123'
...
BEGIN
    ...
    -- Remove [ and ] from JSON array
    SET @json = REPLACE(REPLACE(@json, '[', ''), ']', '');
    
    -- Cursor for columns of the table (matching order)
    ...
    BEGIN
        ...
        ELSE IF @dtype IN ('datetime', 'datetime2', 'smalldatetime', 'date', 'datetimeoffset')
            SET @result = @result + '''' + @val + ''',';
        ELSE
...
-- fn_json_to_set_clause: Converts JSON to SET clause for UPDATE
-- e.g. '{"col1":"newval","col2":456}' → 'col1 = 'newval', col2 = 456'
...
-- fn_json_to_where_clause: Converts JSON to WHERE clause
-- Uses all non-null values as AND conditions
